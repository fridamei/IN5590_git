...
% Make voxel body (tested with rod and box)
vG = voxBox(boxXlength, boxYlength, adjustedHeight, 'voxSize', voxDim);
%vG = voxRod(boxXlength, adjustedHeight, 'voxSize', voxDim);
...
fid = fopen('gcode_model.gcode', 'w', 'n','UTF-8');
...
for i = 1:nLayers
    ...
    for j = 1:nLines
        ...
        % Update E-value
        eTotal = eTotal + (xDist * horizontalLayer) + (yDist * verticalLayer);
